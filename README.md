...
- 从任意已登录 GitLab 页面识别当前实例，不推断 Group 或 Project 路径
- 扫描当前账号具备 Maintainer 或 Owner 权限的项目
- 自动处理超过 100 个项目的 GitLab API 分页
- 显示实例、项目数量及完整 Repo 路径
- 默认勾选全部项目，支持逐项选择、全选、取消全选和半选状态
- 支持按 `path_with_namespace` 搜索项目
- 输入 Webhook URL，可选填写 Secret Token
...
4. 点击“检查可配置项目”。
5. 查看项目总数和完整 Repo 路径，搜索并取消不需要处理的项目。
6. 点击“预览选中项目”，确认只查询所选项目的 Webhook。
7. 点击“确认执行”后批量写入。

扫描和预览阶段不会修改项目配置；只有点击“确认执行”后才会写入 Webhook。如果没有选择项目，预览和执行都会被阻止。预览后修改勾选状态会使旧预览立即失效，必须重新预览。如果缺少 Webhook URL，Popup 会提示先打开设置填写。Secret Token 是可选项。
...
右键点击浏览器工具栏中的“Webhook 一键配置”图标，选择“选项”，可以保存默认 Webhook URL，并可选保存 Secret Token。

Popup 底部可以下载本次操作日志，包括扫描、预览、执行摘要和实际写入请求摘要。Webhook 创建与更新日志同时记录完整 Repo 路径、Project ID 和适用的 Hook ID，便于人工识别及 API 故障排查。日志不会包含 Secret Token 明文。
...
- 无权访问的项目可能返回 `403` 或 `404`
- 会话失效后需要先重新登录 GitLab

为了支持任意自托管 GitLab 地址，开发版需要申请 HTTP 与 HTTPS 站点访问权限。扩展只会根据当前页面的 origin 识别实例地址，并通过 `/api/v4/user` 验证登录状态。项目扫描固定使用 `/api/v4/projects?membership=true`，不会根据当前 Group、Subgroup 或 Project 路径缩小范围。

## 安全原则

- Webhook URL 和可选 Secret Token 保存在浏览器扩展本地存储中
...
测试覆盖实例 URL 识别、API 错误映射、项目分页、项目选择与预览快照、Webhook 配置校验、精确 URL 匹配、安全操作日志、预览计划、写入执行和 Popup 状态数据。
