...
扫描和自动预览阶段不会修改项目配置；只有点击“确认执行”后才会写入 Webhook。如果缺少 Webhook URL，Popup 会提示先打开设置填写。Secret Token 是可选项。
...
Popup 底部可以下载本次操作日志，包括扫描、预览、执行摘要和实际写入请求摘要，日志不会包含 Secret Token 明文。
